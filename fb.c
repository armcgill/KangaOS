...
{
    int offs = (y * pitch) + (x * 4);       //calculate the frame buffer offset resulting in (x,y)
    *((unsigned int*)(fb + offs)) = vgapal[attr & 0x0f]; //set that byte to the framebuffer to the foreground color
}


//Taken from Breshenhams Line drawing Algorithm
