...
enum {  
    GPFSEL0         = PERIPHERAL_BASE + 0x200000,   //GIPO Function select 0 
    GPSET0          = PERIPHERAL_BASE + 0x20001C,   //GIPO Pin output set 0
    GPCLR0          = PERIPHERAL_BASE + 0x200028,   //GIPO Pin Output clear 0
    GPPUPPDN0       = PERIPHERAL_BASE + 0x2000E4    //GIPO pull-up/pull-down register 0 -- gives the current pull state
};
...
void mmio_write(long reg, unsigned int val) { *(volatile unsigned int *)reg = val; } //takes val and assigns it to a volitile pointer reg
unsigned int mmio_read(long reg) { return *(volatile unsigned int *)reg; } //Returns actual value in reg 
...
enum {
    ...
    AUX_MU_IO_REG   = AUX_BASE + 64,    //writes data to and reads data from UART FIFOs
    AUX_MU_IER_REG  = AUX_BASE + 68,    //Enables interrupts
    AUX_MU_IIR_REG  = AUX_BASE + 72,    //Shows the interrupt status
    AUX_MU_LCR_REG  = AUX_BASE + 76,    //Controls line sata format and gives access to the baudrate register
    AUX_MU_MCR_REG  = AUX_BASE + 80,    //Controls the modem signals
    AUX_MU_LSR_REG  = AUX_BASE + 84,    //Shows the data status
    AUX_MU_MSR_REG  = AUX_BASE + 88,
    AUX_MU_SCRATCH  = AUX_BASE + 92,
    AUX_MU_CNTL_REG = AUX_BASE + 96,
    AUX_MU_STAT_REG = AUX_BASE + 100,    
    AUX_MU_BAUD_REG = AUX_BASE + 104,   //Allows direct access to the 16-bit baudrate counter
    AUX_UART_CLOCK  = 500000000,        // same clock speed as 500Mhz
    UART_MAX_QUEUE  = 16 * 1024
};

#define AUX_MU_BAUD(baud) ((AUX_UART_CLOCK/(baud*8))-1)     //Calculates baudrate
...
unsigned int uart_isWriteByteReady() { return mmio_read(AUX_MU_LSR_REG) & 0x20; }   //checks the UART line status to ensure it is "ready to send"
...
void uart_writeByteBlockingActual(unsigned char ch) {       //sends a single character
...
void uart_writeText(char *buffer) { //Sends a whole string 
    while (*buffer) {
